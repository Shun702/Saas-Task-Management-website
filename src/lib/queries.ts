...
if (!foundAgencyId){
    if(!subaccountId){
        throw new Error(
            'You need to provie at lease an agency Id or subaccount Id'
        )
    }
...
export const verifyAndAcceptInvitation = async() => {
    ...
    const invitationExists = await db.invitation.findUnique({ where: {email:user.emailAddresses[0].emailAddress,
status: 'PENDING',
},
})
...
if (invitationExists){
    const userDetails = await createTeamUser(
        invitationExists.agencyId,{email:invitationExists.email,
...
export const _getTicketsWithAllRelations = async (landId: string) => {
    const response = await db.ticket.findMany({
        where: { laneId: landId },
        include: {
            Assigned: true,
